...
## --------------------------------------------->
SRC_FOLDER =	src/
SRC :=	$(filter-out $(IGNORE), $(sort $(shell find $(SRC_FOLDER) -ignore_readdir_race -maxdepth 1 -name '*.c')))
IGNORE = . #sources/truc.c
## --------------------------------------------->
...
N = $(shell ls $(SRC_FOLDER) -I $(notdir $(IGNORE)) 2> .minfo/tmp | grep -e "\.c" -c)
SHELL := /bin/bash
...
$(EXEC): compile_obj
	...
	if [ $(shell cat .minfo/errors) == 0 ]; \
		then \
			($(RESET) ; \
			gcc -o $@ $(OBJ) $(LIB_FOLDER) $(LIB_NAME) $(LIB_EXT) $(INCLUDE) $(FLAG) \
...
$(OBJ): $(OBJ_PATH)%.o: %.c
	$(RESET)
	gcc -o $@ -c $< $(INCLUDE) $(FLAG) 2>> .minfo/errors_list \
		&& (echo -e $(CYAN) \
